...
你是学习者画像分析师。输入问卷与对话摘要，输出 JSON 对象 learner_profile：
{
  ...
  "topic_signals": ["用户在对话中提到过的话题关键词..."],
  "source_stats": { "from_questionnaire": int, "from_dialogue": int, "from_materials": 0 },
  "web_enabled": false,
  "generated_at": "<ISO-8601>"
}
要求：具体、可引用、不要总结成形容词；弱项必须带证据；install-only 模式下 from_materials 恒为 0。
```
...
基于 learner_profile，输出 capability_framework：
{
  "source": "llm",
  "dimensions": {
    "ability":  ["fluency", "lexical_range", "grammar_range", "pronunciation", ...],
    "scenario": ["IELTS P1", "IELTS P2", "IELTS P3", "daily conversation", ...],
    "goal":     ["reach 7.0 by <exam_window>", ...]
  },
  "pain_points": [
    { "id": "PP-01", "name": "...", "evidence": "引用 language_samples 某一句或 weakness_signals 某一条" }
  ],
  "lift_paths": [
    { "from": "PP-01", "to": "目标能力描述", "band_gap": "6.5→7.0" }
  ]
}
每个 lift_path 必须绑定一个或多个 pain_point id；dimensions 三个数组至少各 2 项。
```

**降级**：LLM 失败 → 使用最小可用骨架（与后端 `capability_framework._rule_fallback()` 对齐）：
```json
{ "source": "rule_fallback",
  "dimensions": { "ability": ["fluency","lexical_range"], "scenario": ["IELTS P1","IELTS P2"], "goal": ["reach target_band"] },
  "pain_points": [{"id":"PP-01","name":"需人工补充","evidence":"rule_fallback"}],
  "lift_paths":  [{"from":"PP-01","to":"practice more","band_gap":"current→target"}] }
```
...
|---|---|---|---|---|
| Stage 1 Research | 无 dialogue 数据 | 从 answers 映射最小 profile；`language_samples=[]` | `from_dialogue=0` | `rule_fallback` |
...
- `learner_profile.web_enabled` / `source_stats.from_materials` 仅 runnable-export 非零
