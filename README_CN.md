...
```bash
# 子路径安装（vercel-labs/skills CLI 只复制该子目录，不会拉 backend/frontend/docs）
...
实际落到 `.agents/skills/personalingo/` 的内容（CLI 只复制仓库的 `skills/personalingo/` 子目录，backend/frontend/docs 不会被拉）：
...
| 安装命令 | `npx skills add orzcls/PersonaLingo --skill personalingo` | `git clone` + `docker-compose up` |
