...
PersonaLingo 将个性化雅思学习能力导出为可移植的 Agent 技能包。
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **A: 使用导出技能** | 加载已蒸馏的技能包（含语料库和提示词） | 快速部署辅导 Agent |
| **B: 完整流程** | 端到端执行：问卷 → 对话 → 蒸馏 → 生成 | 从零创建个性化语料库 |

### 技能包内容

每个导出的技能包包含：
- `Skill.md` — 个性化学习技能文档
- `corpus.json` — 带 QMD 标签的话题语料库
- `runtime_protocol.md` — Agent 运行时协议
- `prompts/` — 对话与评估提示词模板
